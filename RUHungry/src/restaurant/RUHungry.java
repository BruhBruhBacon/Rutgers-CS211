...
public class RUHungry {
    ...
    int catsNum = Integer.parseInt(StdIn.readLine());
    //System.out.println(catsNum);
    categoryVar = new String[catsNum];
    menuVar = new MenuNode[catsNum];
    MenuNode prev = null;
    MenuNode men = null;
    for(int i = 0;i < catsNum;i++){
        String catname = StdIn.readLine();
        //System.out.println(catname + " cat ");
        categoryVar[i] = catname;
        ...
        for(int k = 0;k < dishNum;k++){
            ...
            int IDlen = StdIn.readInt();
            //System.out.println(IDlen);
            int[] IDnums = new int[IDlen];
            for(int s = 0;s < IDlen;s++){
                IDnums[s] = StdIn.readInt();
                //System.out.print(IDnums[s] + " ");
            }
            //System.out.println();
            StdIn.readLine();
            Dish dis = new Dish(catname, dishname, IDnums);
            if(k == 0){
                men = new MenuNode(dis, null);
                prev = men;
            }
            else{
                men = new MenuNode(dis, prev);
                   prev = men;
            }
        }
        menuVar[i] = men;
    ...
    //      java -cp bin restaurant.Menu menu.in menu.out
    ...
    int h = newNode.getIngredient().getID() % stockVarSize;
    if(stockVar[h] == null){
        stockVar[h] = newNode;
    }
    else{
        StockNode prev = stockVar[h];
        newNode.setNextStockNode(prev);
        stockVar[h] = newNode;
    }
    ...
    if(ingredientName == null){ 
        StockNode found = findStockNode(ingredientID);
        found.getIngredient().setStockLevel(found.getIngredient().getStockLevel() + stockAmountToAdd);
    }
    if(ingredientID == -1){
        StockNode found = findStockNode(ingredientName);
        found.getIngredient().setStockLevel(found.getIngredient().getStockLevel() + stockAmountToAdd);
    }
	...
	// WRITE YOUR CODE HERE
    for(MenuNode n:menuVar){
        while(n != null){
            int[] ings = n.getDish().getStockID();
            double cost = 0.0;
            double price = 0.0;
            for(int in:ings){
                cost += findStockNode(in).getIngredient().getCost();
            }
            price = cost*1.2;
            n.getDish().setPriceOfDish(price);
            n.getDish().setProfit(price - cost);
    ...
    Ingredient ingred = new Ingredient(id, name, stock, cost);
    StockNode n = new StockNode(ingred, null);
    addStockNode(n);
    }  
    //javac -d bin src/restaurant/*.java
    ...
    TransactionNode added = new TransactionNode(data, null);

    if(transactionVar == null){
        transactionVar = added;
    }
    else{
        TransactionNode curr = transactionVar;
    ...
    int[] ings = d.getDish().getStockID();
    for(int i =0;i<ings.length;i++){
        StockNode ingre = findStockNode(ings[i]);
        int amount = ingre.getIngredient().getStockLevel();
        if(amount < numberOfDishes){
            return false;
        }
    }
    ...
    MenuNode nod = findDish(dishName);
    if(checkDishAvailability(dishName, quantity) == true){
        TransactionData good = new TransactionData("order", dishName, quantity, nod.getDish().getProfit() * quantity, true);
        addTransactionNode(good);
        for(int ID : nod.getDish().getStockID()){
        ...
        TransactionData bad = new TransactionData("order", dishName, quantity, 0, false);
        addTransactionNode(bad);
        MenuNode curr = nod.getNextMenuNode();
        // Menunode og = nod;
        int loopstopper = 0;
        while(curr != null){
            if(checkDishAvailability(curr.getDish().getName(), quantity) == true){
                TransactionData good = new TransactionData("order", curr.getDish().getName(), quantity, curr.getDish().getProfit() * quantity, true);
                addTransactionNode(good);
                for(int ID : curr.getDish().getStockID()){
                updateStock(null, ID, -quantity);
                }
                return;
            }
            else{
                TransactionData badder = new TransactionData("order", curr.getDish().getName(), quantity, 0, false);
                addTransactionNode(badder);
            }
            
            curr = curr.getNextMenuNode();
        } 
        curr = menuVar[findCategoryIndex(nod.getDish().getCategory())];
        while(curr != nod){
            //lock in


            if(checkDishAvailability(curr.getDish().getName(), quantity)){
                TransactionData good = new TransactionData("order", curr.getDish().getName(), quantity, curr.getDish().getProfit() * quantity, true);
                addTransactionNode(good);
                for(int ID : curr.getDish().getStockID()){
                updateStock(null, ID, -quantity);
                ...
                TransactionData worst = new TransactionData("order", curr.getDish().getName(), quantity, 0, false);
                addTransactionNode(worst);
            }
            curr = curr.getNextMenuNode();
        }
	...
	// WRITE YOUR CODE HERE
    Double sum = 0.0;
    TransactionNode nod = transactionVar;
    while(nod != null){
        sum += nod.getData().getProfit();
        nod = nod.getNext();
    }
    ...
    if(profit() > 50 && (findStockNode(ingredientName).getIngredient().getStockLevel() >= quantity)){
        dtd = new TransactionData("donation", ingredientName, quantity, 0, true);    
        updateStock(ingredientName, -1, -quantity);
    }
    else{
        dtd = new TransactionData("donation", ingredientName, quantity, 0, false);   
    }
    addTransactionNode(dtd);
    ...
    if(profit() >= findStockNode(ingredientName).getIngredient().getCost() * quantity){
        TransactionData dt = new TransactionData("restock", ingredientName, quantity, -(findStockNode(ingredientName).getIngredient().getCost() * quantity), true);
        addTransactionNode(dt);
        updateStock(ingredientName, -1, quantity);

    }
    else{
    
        TransactionData dt = new TransactionData("restock", ingredientName, quantity, 0, false);
        addTransactionNode(dt);
    }
    } 
